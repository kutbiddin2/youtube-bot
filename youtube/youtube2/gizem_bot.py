...
MUZIK_DOSYASI   = r"C:\Users\Talha\OneDrive\Desktop\youtube\youtube2\muzik.mp3"
CIKTI_KLASORU   = r"C:\Users\Talha\OneDrive\Desktop\youtube\youtube2\videolar"
...
def script_uret(konu=None):
    ...
    if konu:
        ...
        kategoriler = [
            "kayıp medeniyetler ve gizemli antik şehirler — Atlantis, El Dorado, Shambhala gibi",
            "çözülemeyen tarihi gizemler — Göbekli Tepe, Nazca çizgileri, Stonehenge gibi",
            "tarihin en büyük gizemleri — kayıp hazineler, gizemli ölümler, açıklanamayan olaylar",
            "antik medeniyetlerin bilinmeyen teknolojileri — piramitler, antik makineler, kayıp bilgi",
        ]
        import random
        secilen = random.choice(kategoriler)
        konu_kismi = f"Türkçe izleyici kitlesine hitap eden merak uyandıran bir konu seç. Kategori: {secilen}. Özgün olsun."

    prompt = f"""Sen bir gizem ve kayıp medeniyetler içerik üreticisisin. Türkçe YouTube videoları için içerik üretiyorsun.
...
{{
  "baslik": "Video başlığı (YouTube tarzı, ilgi çekici)",
  "konu_ozeti": "2-3 cümle özet",
  "script": "Tam video metni. En az 800 kelime. Akıcı sürükleyici belgesel anlatımı.",
  "gorsel_sahneler": [
    {{"sahne": 1, "aciklama": "Görsel 1 açıklaması", "prompt": "Detailed English image description, historical scene, cinematic, dramatic lighting, oil painting style, no text, no watermark"}},
...
def seslendir(metin: str, dosya: str):
    print("\n🎙️  Seslendirme yapılıyor (Gemini TTS)...")
    client = genai.Client(api_key=GEMINI_API_KEY)
...
if __name__ == "__main__":
    ...
    # Konu belirtmek için:
    # main("Kanuni Sultan Süleyman Viyana'yı Alsaydı Ne Olurdu?")
