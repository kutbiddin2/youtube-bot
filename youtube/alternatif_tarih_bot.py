...
MUZIK_DOSYASI   = r"C:\Users\Talha\OneDrive\Desktop\youtube\muzik.mp3"
CIKTI_KLASORU   = "videolar"
...
def script_uret(konu=None):
    ...
    if konu:
        ...
        konu_kismi = "Önce ilgi çekici, daha önce işlenmemiş bir alternatif Türk/İslam tarihi konusu seç."

    prompt = f"""Sen bir alternatif tarih içerik üreticisisin. Türkçe YouTube videoları için içerik üretiyorsun.
...
{{
  ...
  "gorsel_sahneler": [
    ...
    for deneme in range(5):  # 5 kez dene
        try:
            ...
            if deneme < 4:
                bekleme = (deneme + 1) * 30  # 30, 60, 90, 120 saniye
                print(f"   ⚠️  Sunucu meşgul, {bekleme}sn bekleniyor... ({deneme+1}/5)")
                time.sleep(bekleme)
            else:
                raise e
...
def gorselleri_uret(sahneler: list, klasor: str) -> list:
    ...
    for i, sahne in enumerate(sahneler):
        ...
        aciklama = sahne.get('aciklama') or sahne.get('description') or sahne.get('sahne', '')
        print(f"   [{i+1}/{len(sahneler)}] {str(aciklama)[:55]}...")
        gorsel_prompt = sahne.get('prompt') or sahne.get('dalle_prompt') or sahne.get('image_prompt', '')
        gorsel_uret(gorsel_prompt, dosya)
